# A2UI Emit Guide — `genskill.cards@1`

> For genSkill phases. When the client advertises support for catalog
> `genskill.cards@1`, emit a structured card **instead of** the equivalent
...
Rules:
- `catalogId` MUST be `genskill.cards@1`. `component` MUST be one of the 6 below — **never** A2UI standard components (Text/Row/Column/Button/…).
- The block must be strict JSONL: each non-empty line inside ```a2ui must be one complete JSON object that `JSON.parse` can parse.
- Always escape embedded double quotes inside string values as `\"`, or use Chinese quotation marks instead. For example, use `比如\"帮我记一下今天的数据\"` or `比如“帮我记一下今天的数据”`, never `比如"帮我记一下今天的数据"`.
...
```a2ui
{"createSurface":{"surfaceId":"plan-confirm-1","catalogId":"genskill.cards@1"}}
...
`a2ui:genskill.cards@1` (or equivalent A2UI `clientUiCapabilities` with
`catalogId: genskill.cards@1`). Only emit A2UI when that support is present;
