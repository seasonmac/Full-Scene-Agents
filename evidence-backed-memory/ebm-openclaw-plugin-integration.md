...
| 文件 | 角色 |
|------|------|
| `ebm_py/server.py` | Python HTTP sidecar 服务器 |
| `ebm-py-plugin/src/index.ts` | OpenClaw TS 插件（独立包） |
| `~/.openclaw/extensions/ebm-context-engine/src/index.ts` | 已安装的插件实例 |
...
### 启动方式
```bash
python3 -m ebm_py.server --port 18790 --config /path/to/config.json --db /path/to/ebm.sqlite
```
...
关键参数:
- `pythonCommand`: 必须是 Python 3.12+（3.9 不支持 `X | None` 类型语法）
- `sidecarPort`: sidecar HTTP 端口，默认 18790
- `ebmPyPath`: ebm_py 包所在的项目根目录
- `dbPath`: SQLite 数据库路径
...
- SQLite: `~/.openclaw/memory/ebm_py.sqlite`
...
| 死重导出 `core/types.py` | core/ | 仅 `from ebm_py.types import *` |
