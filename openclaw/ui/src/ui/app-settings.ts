...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "tracing") {
    await loadTraces(host as unknown as OpenClawApp, { reset: true });
  }
}
...
function applyTabSelection(
  ...
  if (next === "tracing") {
    startTracingPolling(host as unknown as Parameters<typeof startTracingPolling>[0]);
  } else {
    stopTracingPolling(host as unknown as Parameters<typeof stopTracingPolling>[0]);
  }
