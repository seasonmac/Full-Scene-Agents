...
vi.mock("./app-polling.ts", () => ({
  ...
  startTracingPolling: vi.fn(),
  stopTracingPolling: vi.fn(),
}));
...
describe("handleConnected", () => {
  ...
  it("starts tracing polling when tracing tab is active", () => {
    loadBootstrapMock.mockResolvedValueOnce(undefined);
    const host = createHost();
    host.tab = "tracing";
    ...
    return import("./app-polling.ts").then((polling) => {
      expect(polling.startTracingPolling).toHaveBeenCalledTimes(1);
    });
  });
});
