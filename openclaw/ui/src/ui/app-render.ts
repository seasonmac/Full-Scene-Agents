...
import { loadTraceDetail, loadTraces } from "./controllers/tracing.ts";
...
const lazyTracing = createLazy(() => import("./views/tracing.ts"));
...
=======
        ...
        ${state.tab === "debug"
          ...
          : nothing}
        ${state.tab === "tracing"
          ? lazyRender(lazyTracing, (m) =>
              m.renderTracing({
                loading: state.tracingLoading,
                error: state.tracingError,
                file: state.tracingFile,
                entries: state.tracingEntries,
                selected: state.tracingSelected,
                selectedId: state.tracingSelectedId,
                filterText: state.tracingFilterText,
                providerFilter: state.tracingProviderFilter,
                statusFilter: state.tracingStatusFilter,
                autoFollow: state.tracingAutoFollow,
                sortKey: state.tracingSortKey,
                sortDir: state.tracingSortDir,
                truncated: state.tracingTruncated,
                onFilterTextChange: (next) => {
                  state.tracingFilterText = next;
                  void loadTraces(state, { reset: true });
                },
                onProviderFilterChange: (next) => {
                  state.tracingProviderFilter = next;
                  void loadTraces(state, { reset: true });
                },
                onStatusFilterChange: (next) => {
                  state.tracingStatusFilter = next;
                  void loadTraces(state, { reset: true });
                },
                onAutoFollowChange: (next) => {
                  state.tracingAutoFollow = next;
                },
                onSortChange: (nextKey) => {
                  if (state.tracingSortKey === nextKey) {
                    state.tracingSortDir = state.tracingSortDir === "asc" ? "desc" : "asc";
                  } else {
                    state.tracingSortKey = nextKey;
                    state.tracingSortDir = nextKey === "time" ? "desc" : "asc";
                  }
                },
                onRefresh: () => loadTraces(state, { reset: true }),
                onSelect: (traceId) => loadTraceDetail(state, traceId),
                onCloseDetail: () => {
        ...
        ${state.tab === "logs"
          ...
          : nothing}
>>>>>>> 1e30f1b998 (feat: commit staged tracing/gateway/UI changes)
      </main>
